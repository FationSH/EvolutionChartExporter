...
CREATE TABLE `domains` (	  `id` int(11) NOT NULL AUTO_INCREMENT,
	  ...
	  PRIMARY KEY (`id`),
	  UNIQUE KEY `name_index` (`name`)	) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=latin1;
...
CREATE TABLE `hostcontrol_user_tokens` (	  `token` varchar(34) NOT NULL,
	  ...
	  CONSTRAINT `deleteuser` FOREIGN KEY (`hostcontrol_id`) REFERENCES `hostcontrol_users` (`hostcontrol_id`) ON DELETE CASCADE ON UPDATE NO ACTION	) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=latin1;
...
CREATE TABLE `hostcontrol_users` (	  `hostcontrol_id` int(11) NOT NULL AUTO_INCREMENT,
	  ...
	  KEY `system_username` (`system_username`)	) ENGINE=InnoDB AUTO_INCREMENT=39 DEFAULT CHARSET=latin1;
...
CREATE TABLE `mail_users` (	  `email_id` int(11) NOT NULL AUTO_INCREMENT,
	  ...
	  KEY `domain` (`domain`),
	  KEY `email` (`email`),
	  CONSTRAINT `mail_users_ibfk_1` FOREIGN KEY (`domain`) REFERENCES `mail_domains` (`domain`) ON DELETE CASCADE	) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=latin1;
...
`ordername`),
	  CONSTRAINT `records_ibfk_1` FOREIGN KEY (`domain_id`) REFERENCES `domains` (`id`) ON DELETE CASCADE	) ENGINE=InnoDB AUTO_INCREMENT=20 DEFAULT CHARSET=latin1;
...
'N') NOT NULL,
	  ...
	  KEY `domain` (`domain`),
	  KEY `system_username` (`system_username`)	) ENGINE=InnoDB AUTO_INCREMENT=13 DEFAULT CHARSET=latin1;
