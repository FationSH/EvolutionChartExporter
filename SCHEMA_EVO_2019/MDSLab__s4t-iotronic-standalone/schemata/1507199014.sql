...
CREATE TABLE IF NOT EXISTS `coordinates` (	  `coordinates_id` INT NOT NULL AUTO_INCREMENT,
	  ...
	  CONSTRAINT `fk_coordinates_boards1`	    FOREIGN KEY (`board_id`)	    REFERENCES `boards` (`board_code`)	    ON DELETE CASCADE	    ON UPDATE CASCADE)	ENGINE = InnoDB	DEFAULT CHARACTER SET = utf8;
