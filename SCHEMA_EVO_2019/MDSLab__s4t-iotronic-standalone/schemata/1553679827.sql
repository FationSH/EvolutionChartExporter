...
CREATE TABLE IF NOT EXISTS `sensors` (	  `id` INT NOT NULL,
	  ...
	  CONSTRAINT `fk_sensors_boards1`	    FOREIGN KEY (`board_id`)	    REFERENCES `boards` (`board_id`)	    ON DELETE NO ACTION	    ON UPDATE NO ACTION)	ENGINE = InnoDB	DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `connectivity_types` (	  `id` INT NOT NULL AUTO_INCREMENT,
	  `c_type` VARCHAR(45) NOT NULL,
	  PRIMARY KEY (`id`),
	  UNIQUE INDEX `c_type_UNIQUE` (`c_type` ASC))	ENGINE = InnoDB	DEFAULT CHARACTER SET = utf8;

CREATE TABLE IF NOT EXISTS `connectivity` (	  `conn_id` INT NOT NULL,
	  `board_id` VARCHAR(36) NOT NULL,
 ...
 `board_id`),
	  ...
	  CONSTRAINT `fk_connectivity_connectivity1`	    FOREIGN KEY (`conn_id`)	    REFERENCES `connectivity_types` (`id`)	    ON DELETE NO ACTION	    ON UPDATE NO ACTION,
	  CONSTRAINT `fk_connectivity_boards1`	    FOREIGN KEY (`board_id`)	    REFERENCES `boards` (`board_id`)	    ON DELETE CASCADE	    ON UPDATE CASCADE)	ENGINE = InnoDB	DEFAULT CHARACTER SET = utf8;
